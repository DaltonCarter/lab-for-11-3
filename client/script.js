...
sayHelloButton.addEventListener('mouseover', styleEdit)
...
function styleRevert(event) {
    event.target.style.color = "black"
      event.target.style.backgroundColor = "#EFEFEF"
}

sayHelloButton.addEventListener('mouseout', styleRevert)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/`<("<) (>")> O(^-^)O`')
    .then(res => {
        console.log(res.data)
        document.querySelector('#repeat-text').textContent = res.value
        document.querySelector('#repeat-text').style.display = 'block'

    })
    console.log('Hey~ Who spiked the eggnog?')
}
...
function sendQuery(){
    axios.get('http://localhost:3000/query-test/?Auir')
    // axios.get('http://localhost:3000/query-test/?name=stark&age=30')
    // axios.get('http://localhost:3000/query-test?')
    .then(res => {
        console.log(res.data)
    
    })
    console.log('The Queen of Blades has risen!')
}
...
/*
    ...
    1: Send no queries on the URL -- what happened?
         My response: *It informed me that i sent no querys.*

    2: Send more than 1 query on the URL -- what happened? 
    
        My Response *I tried two things the first had two ?and then a thing, it spit mack a notice that i sent a query for both things as if they were one thing. The second thing I did was to just run two at once and it only ran the bottom one, tried a third thing it informed me I sent more than one request.
    I have NO IDEA what was supposed to happen here, because i was ecpecting a catch error or something.*
*/
